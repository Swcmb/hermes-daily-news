...
description: 智讯日报主编 - 每日自动推送综合新闻/AI科技/学术三大日报到 QQ bot（v2.0 Shell+Python 混合架构）
version: 2.0.0
author: 智讯日报主编 Agent
---
...
每日自动从 16 个数据源采集并推送三大类日报到用户 QQ：

| 日报类型 | 推送时间 | 模式 | 数据源 |
|---------|---------|------|--------|
| 📰 综合新闻日报 | 每日 08:00 | no-agent（纯 Shell） | 知乎热榜 · 36氪快讯 · 牛客热议 · 微博热搜 · 百度热搜 |
| 📡 AI 科技日报 | 每日 12:00 | agent（LLM 翻译） | arXiv cs.AI/cs.LG · GitHub · Hacker News · Product Hunt · Reddit r/ML |
| 🧬 学术日报 | 每日 18:00 | agent（LLM 翻译） | arXiv cs.AI/cs.LG/stat.ML/q-bio.GN/q-bio.QM · PubMed · bioRxiv |

## 设计原则

- **中文优先**：所有推送内容必须为简体中文。英文源（arXiv/GitHub/HN/Reddit/ProductHunt/PubMed/bioRxiv）的标题和摘要必须翻译后呈现。用户明确要求"用中文"时此规则强制执行
...
- **专业术语保留英文**：LLM、Agent、GAN、Transformer、BERT、GNN、CNN、RNN、LSTM、RLHF、RAG 等通用术语保留英文原文
- **人名保留英文**：作者姓名保留英文原文
...
---
⏱ 数据采集时间：2026-07-10T12:00:00+08:00 | 智讯日报 v2.0
```
...
# 推送到 QQ
hermes send -f /tmp/news-tech-ai-${DATE}.md -t qqbot:AC6557CF43ED1A86EA5C1A17C72B5B6D
...
- 缓存：`/tmp/hermes-news-cache/`（sha1(url) 为 key，TTL 按源类型区分）
...
### tech-ai（英文源，agent 模式 LLM 翻译）

| 源 | URL | 条数 |
|----|-----|------|
| arXiv cs.AI | rss.arxiv.org/rss/cs.AI | 6 |
| arXiv cs.LG | rss.arxiv.org/rss/cs.LG | 6 |
| GitHub AI | api.github.com/search/repositories（topic:llm+OR+topic:agent） | 8 |
| Hacker News | hn.algolia.com/api/v1/search?tags=front_page | 10 |
| Product Hunt | producthunt.com/feed | 6 |
| Reddit r/ML | reddit.com/r/MachineLearning/top.json?t=day | 6 |
